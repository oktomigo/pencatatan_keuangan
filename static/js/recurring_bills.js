...
import { formatRupiah, daysFromNow, today, rupiahInput, rupiahInputValue } from './utils.js';
...
function openBillForm(bill = null) {
  ...
  setTimeout(() => {
    ...
    const amountInput = form.querySelector('#bill-amount'); rupiahInput(amountInput);
    ...
    form.addEventListener('submit', async event => {
      ...
      const name = form.querySelector('#bill-name').value.trim(); const amount = rupiahInputValue(amountInput); const freq = frequency.value; const dueDay = Number(form.querySelector('#bill-due-day').value); const dueDate = form.querySelector('#bill-due-date').value.trim();
      const errors = {}; if (!name) errors.name = 'Nama tagihan wajib diisi'; if (isNaN(amount) || !Number.isInteger(amount) || amount <= 0 || amount > MAX_AMOUNT) errors.amount = 'Nominal harus berupa angka lebih dari 0 dan tidak melebihi 999.999.999.999'; if (freq === 'yearly' ? !/^((0[1-9])|(1[0-2]))-(0[1-9]|[12][0-9]|3[01])$/.test(dueDate) : !Number.isInteger(dueDay) || dueDay < 0 || dueDay > (freq === 'weekly' ? 6 : 31)) errors.due_day = 'Tanggal jatuh tempo tidak valid';
